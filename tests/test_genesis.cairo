...
fn test_genesis_constructor() {
    ...
    assert(genesis.total_minted() == 0, 'total_minted should be 0');
...
fn test_mint_single() {
    ...
    // Verify
    assert(genesis.total_minted() == 1, 'should have minted 1');
...
fn test_mint_batch() {
    ...
    assert(genesis.total_minted() == 3, 'should have minted 3');
...
fn test_admin_mint() {
    ...
    assert(genesis.total_minted() == 10, 'should have minted 10');
...
fn test_admin_mint_exceeds_remaining_reverts() {
    ...
    start_cheat_caller_address(genesis_address, OWNER());
    genesis.admin_mint(MINTER(), 501); // max_supply is 500
}
...
fn test_admin_mint_no_batch_limit() {
    ...
    assert(genesis.total_minted() == 50, 'should have minted 50');
}
...
fn test_mint_after_sold_out_reverts() {
    ...
    // Mint 50, then try to mint 451 (would exceed 500)
    start_cheat_caller_address(genesis_address, OWNER());
    genesis.admin_mint(MINTER(), 50);
    genesis.admin_mint(MINTER(), 451); // should panic: exceeds remaining
}
